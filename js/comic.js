(function (imageproc) {
    ...
    imageproc.comicColor = function (inputData, outputData, saturation) {
        ...
        for (var i = 0; i < inputData.data.length; i += 4) {
            ...
            var hsvValue = imageproc.fromRGBToHSV(r, g, b)

            // then, increase the saturation by the saturation factor
            hsvValue.s *= saturation;

            // ***** beware of the final range of the saturation *****
            if (hsvValue.s > 1)
                hsvValue.s = 1;
            else if (hsvValue.s < 0)
                hsvValue.s = 0;

            // after that, convert it back to RGB
            var rgbValue = imageproc.fromHSVToRGB(hsvValue.h, hsvValue.s, hsvValue.v)
            ...
            for (let j = 0; j < palette.length; j++) {
                var colorDistance = Math.hypot(palette[j][0] - rgbValue.r, palette[j][1] - rgbValue.g, palette[j][2] - rgbValue.b);
